...
import { DoctorDTO, PersonalType } from '@/types/personal';
import { ROUTES, MESSAGES } from '@/constants';

const DoctorCreatePage: React.FC = () => {
  const navigate = useNavigate();
  const [alert, setAlert] = useState<{ type: 'success' | 'error'; message: string } | null>(null);

  const { specialities } = useSpecialities();

  const handleSubmit = async (data: DoctorDTO) => {
    try {
                ...
                <ol className="flex items-center space-x-4">
                  ...
                  <li>
                    <div className="flex items-center">
                      ...
                      <span className="ml-4 text-gray-500">Registro de Doctor</span>
                    </div>
                  </li>
                </ol>
              </nav>
              <h1 className="mt-2 text-2xl font-bold text-gray-900">Registro de Doctor</h1>
              <p className="mt-1 text-sm text-gray-500">
                Complete la información requerida para registrar un nuevo doctor en el sistema
              </p>
            </div>
          </div>
        </div>
