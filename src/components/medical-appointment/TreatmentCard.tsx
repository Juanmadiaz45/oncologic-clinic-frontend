...
import { PlusIcon, BeakerIcon, CalendarIcon } from '@heroicons/react/24/outline';
import { 
  Treatment, 
  PrescribedMedicine, 
...
const TreatmentCard: React.FC<TreatmentCardProps> = ({ treatment }) => {
  ...
  const loadMedicines = async () => {
    try {
      setIsLoading(true);
      const medicinesData = await medicalAppointmentService.getTreatmentMedicines(treatment.id);
            ...
            <form onSubmit={handleSubmitMedicine} className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewMedicine(prev => ({ ...prev, frequencyOfAdministration: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-clinic-500 focus:border-clinic-500 text-sm"
              ...
              <div className="flex space-x-2">
                <Button
                  ...
                  variant="primary"
                  size="sm"
                  disabled={!newMedicine.medicine.trim() || !newMedicine.dose.trim() || isLoading}
                  isLoading={isLoading}
                >
                  Agregar Medicina
                </Button>
        ...
        <div className="space-y-3">
          {isLoading && medicines.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-6 text-clinic-500">
              ...
              <p className="text-sm text-gray-500">No hay medicinas prescritas</p>
            </div>
          ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-gray-600">
                      <div>
                        <span className="font-medium">Dosis:</span> {medicine.dose}
                      </div>
                      {medicine.frequencyOfAdministration && (
                        <div>
                          <span className="font-medium">Frecuencia:</span> {medicine.frequencyOfAdministration}
                        </div>
                      )}
                      {medicine.duration && (
                        <div>
                          <span className="font-medium">Duración:</span> {medicine.duration}
                        </div>
                      )}
                    </div>
                    {medicine.instructions && (
                      <div className="mt-2 text-sm text-gray-700">
                        <span className="font-medium">Instrucciones:</span> {medicine.instructions}
                      </div>
                    )}
                  </div>
